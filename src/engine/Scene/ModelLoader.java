...
public class ModelLoader {
    ...
    private static Material processMaterial(AIMaterial aiMaterial, String modelDir, TextureCache textureCache){
        ...
        try (MemoryStack stack = MemoryStack.stackPush()){
            ...
            float[] shiniessFactor = new float[]{0.0f};
            int[] pMax = new int[]{1};
            result = aiGetMaterialFloatArray(aiMaterial, AI_MATKEY_SHININESS_STRENGTH, aiTextureType_NONE, 0, shiniessFactor, pMax);
            if(result == aiReturn_SUCCESS){
                reflectance = shiniessFactor[0];
            }
    ...
    private static Mesh processMesh(AIMesh aiMesh, List<Bone> boneList){
        float[] vetices = processVertices(aiMesh);
        ...
        if(textCoords.length == 0){
            int numElements = (vetices.length / 3) * 2;
            textCoords = new float[numElements];
        }
        return new Mesh(vetices, normals, tangents, bitangents,textCoords, indices, animMeshData.boneIDs, animMeshData.weights);
    }
    ...
    private static float[] processBitangents(AIMesh aiMesh, float[] normals){
        ...
        int pos = 0;
        while (buffer.remaining() > 0){
            AIVector3D aiBitagent = buffer.get();
            data[pos++] = aiBitagent.x();
            data[pos++] = aiBitagent.y();
            data[pos++] = aiBitagent.z();
        }
